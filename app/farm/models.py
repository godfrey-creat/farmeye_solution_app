...
class PestControl(db.Model):
    ...
    detected_by = db.Column(db.String(50), default='Manual')  # Manual, YOLO, Sensor, etc.
...
class PestAction(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    pest_control_id = db.Column(db.Integer, db.ForeignKey('pest_control.id'), nullable=False)
    action_type = db.Column(db.String(50), nullable=False)  # Chemical, Biological, Mechanical, etc.
    ...
    effectiveness = db.Column(db.String(20), nullable=True)  # Low, Medium, High (to be filled after evaluation)
    cost = db.Column(db.Float, nullable=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)
    
    # Use string reference for User
    user = db.relationship('User', backref='pest_actions', lazy=True, foreign_keys=[user_id])
